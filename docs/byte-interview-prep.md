...
**第一，父子块分层 vs 单层切片。** 单层切片简单、存储少，但检索粒度和上下文不可兼得——小块检索准但缺上下文，大块上下文全但检索粗。医疗场景回答必须上下文完整，单层切片两个方向都留缺陷，父子块多存一倍数据是值得的代价。

**第二，MarkdownHeaderTextSplitter vs 纯 RecursiveCharacterTextSplitter。** 按标题切的优势是切分边界天然是语义边界，一块一个主题；缺点是依赖文档有标题结构。纯按长度切不依赖文档格式，但语义边界不可控，可能一句话被切成两半。我们的知识库来源是 MedlinePlus、NHC、WHO 的 Markdown 文档，标题结构规范，按标题切是利用了这个优势。如果未来接入无结构文本（比如用户粘贴的病历），需要加语义切分或降级到 RecursiveCharacterTextSplitter。

**第三，overlap 100 vs 无 overlap / 大 overlap。** 无 overlap 会导致边界信息丢失；overlap 200+ 冗余更多，检索时同一文档可能被多个子块命中，排名计算受干扰。100 是经验值，逻辑上是"刚好覆盖一个完整句子的长度"，医疗文档中文句子通常在 50-100 字符，100 够用。

**有没有做消融实验？** 没有做严格消融。这些选择是基于设计推理——父子块的理论优势明确，标题切分利用了文档结构特征，overlap 是经验值。如果要严格验证，应该固定其他变量，分别用不同方案跑同一组检索测试集，对比 Precision@5。当时优先保证端到端效果，没有单独对切片策略做消融。
...
### Q32：和市面上普通医疗智能客服比，你的优势在哪？

**参考回答：**

市面上典型的医疗智能客服是**单轮 FAQ 匹配**——用户问一个问题，系统从知识库里找最像的 Q&A 对返回。好一点的加了意图分类，但意图之间是割裂的，不会跨意图流转。再好一点的用纯 LLM 对话，但所有意图走同一条链路，预约和问答混在一起处理。我的优势在三个点：

**第一，多意图路由 + 跨意图续接，不是单链路问答。** 普通智能客服要么只会问答（FAQ 匹配），要么只会办事（规则流程），两个割裂。用户在预约流程中问了个医学问题，传统系统要么忽略继续引导预约，要么丢失预约状态切到问答。我的系统是三条链路独立处理 + 共享会话状态——医学问题走 RAG 回答，预约的 pending 状态不清除，用户回来还能继续挂号。这不是简单的"能问答也能预约"，是多轮多意图并行不丢失。

**第二，RAG 不是简单的向量检索 + 生成，是完整检索工程链。** 普通智能客服的 RAG 是"用户问题 → embedding → 相似度检索 → 喂给 LLM"。我的链路是 query rewrite → 多路检索规划 → 混合检索（向量+全文 RRF 融合）→ 分层检索（按 source_type 优先级）→ rerank 精排 → 相关性分级 → 充足性检查（不够自动换 query 重试）→ grounding 检查。每一个环节都有兜底——检索不到走 fallback 带免责声明，不会编造信息。这不是调个 API 的事，是检索质量的可控工程。

**第三，安全可控，不是 LLM 自由发挥。** 普通智能客服如果用纯 LLM，回答不可控——可能给错误医学建议，可能被 prompt injection 绕过。我的系统关键操作不依赖 LLM 判断：意图路由有规则层兜底，预约确认是代码级关键词校验不是 LLM 判断，高风险症状强制插入就医提醒是规则层做的，回答生成后有 grounding check。LLM 只负责理解和生成，写库的钥匙和安全的底线在代码手里。

**总结**：市面上的医疗智能客服要么是单轮 FAQ 匹配不够智能，要么是纯 LLM 对话不够安全。我的方案是在智能和安全之间找平衡——用 LLM 处理语义理解，用代码控制关键路径，三条链路可以交叉但不混乱。

---

### Q33：如果用户量涨 10 倍，你系统哪里先扛不住？
...
### Q36：如果让你加一个新功能（比如"查看检查报告"），你会怎么扩展路由？
