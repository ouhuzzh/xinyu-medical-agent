...
Docker 部署通过 `project/worker.py` 运行独立知识库 Worker。自动补建和定时同步
分别由 `KB_WORKER_BOOTSTRAP_ON_START`、`KB_WORKER_SYNC_ENABLED` 控制；Compose
会把原有的 `AUTO_BOOTSTRAP_KNOWLEDGE_BASE`、`ENABLE_KB_SYNC_SCHEDULER` 映射给
Worker，并在 API 容器内强制关闭这两个开关。
